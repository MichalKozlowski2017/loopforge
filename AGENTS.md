...
- Route history in the current MVP is stored in browser `localStorage`; server-side `/api/routes` persistence is intentionally disabled (returns 403) unless `DATABASE_URL` is set.
