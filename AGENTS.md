...
- **Routing engine (important, non-obvious):** the generator needs a routing backend. The simplest local path is **BRouter** (uses the preinstalled Java). BRouter binaries/segments live under `infra/brouter/` and `apps/web/.env.local`, both of which are **gitignored** and are NOT restored by a git pull. If `infra/brouter/` is missing, run `pnpm setup:brouter:minimal` once (downloads a ~150 MB Warsaw-area segment set from external hosts and writes `apps/web/.env.local`). Use `pnpm setup:brouter:poland` for Poland, `pnpm setup:brouter:europe` (~3 GiB), or `pnpm setup:brouter:planet` for the full world set (~9 GiB). Because segments are gitignored, **generated routes only work in areas whose `.rd5` segments were downloaded** (minimal = Warsaw / central Poland). Test start points there (e.g. Warszawa ~52.23, 21.01) unless planet/europe segments are installed.
